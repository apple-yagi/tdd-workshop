...
class ClosedRangeTest extends TestCase
{
    ...
    {
        return [
            "整数閉区間は文字列表記の[3,7]が返される" => [3, 7, '[3,7]'],
            "整数閉区間は文字列表記の[1,100]が返される" => [1, 100, '[1,100]'],
            "整数閉区間は文字列表記の[2,99]が返される" => [2, 99, '[2,99]'],
        ];
    }
    ...
    {
        return [
            "整数閉区間は4を渡すとtrueが返される" => [3, 7, 4],
            "整数閉区間は1を渡すとtrueが返される" => [1, 100, 1],
            "整数閉区間は99を渡すとtrueが返される" => [2, 99, 99],
        ];
    }
    ...
    {
        return [
            "整数閉区間は10を渡すとfalseが返される" => [3, 7, 10],
            "整数閉区間は0を渡すとfalseが返される" => [1, 100, 0],
            "整数閉区間は100を渡すとfalseが返される" => [2, 99, 100],
        ];
    }
